...
from jax.tree_util import tree_map as tree_multimap
...
def momentum(step_size, f_fn, mass=0.0):
    ...
    def update(i, state, batch, weights):
        ...
        v = tree_multimap(v_update, v, g)
        x = tree_multimap(x_update, x, v)
        return x, v
...
def nesterov(step_size, f_fn, mass=0.0):
    ...
    def update(i, state, batch, weights):
        ...
        g = grad(f_fn)(x, batch, weights)
        v = tree_multimap(v_update, v, g)
        x = tree_multimap(x_update, x, v, g)
        return x, v
    # Get parameters function
    def get_params(state):
        x, _ = state
...
def rmsprop(step_size, f_fn, gamma=0.9, eps=1e-8, momentum=0.0):
    ...
    def update(i, state, batch, weights):
        ...
        # Perform update
        g = grad(f_fn)(x, batch, weights)
        a = tree_multimap(a_update, a, g)
        m = tree_multimap(m_update, m, g, a)
        x = tree_multimap(x_update, x, m)
        return x, a, m
    # Get parameters function
    def get_params(state):
        x, _, _ = state
...
def adam(step_size, f_fn,
    ...
    def update(i, state, batch, weights):
        ...
        # Perform update
        g = grad(f_fn)(x, batch, weights)
        m = tree_multimap(m_update, m, g)
        v = tree_multimap(v_update, v, g)
        mhat = tree_multimap(mhat_upate, m)
        vhat = tree_multimap(vhat_upate, v)
        g = tree_multimap(g_update, mhat, vhat)
        x = tree_multimap(x_update, x, g)
        cnt += 1
        return x, m, v, cnt
    # Get parameters function
    def get_params(state):
...
def mdmm_adam(step_size, objective, constraints, epsilon,
    ...
    def update(i, state, batch, weights):
        ...
        damp = tree_multimap(d_update, cons, epsilon)
        weights = tree_multimap(w_update, lam, damp)
        g = grad(f_fn)(x, batch, weights)
        m = tree_multimap(m_update, m, g)
        v = tree_multimap(v_update, v, g)
        mhat = tree_multimap(mhat_upate, m)
        ...
        g = tree_multimap(g_update, mhat, vhat)
        x = tree_multimap(x_update, x, g)
        lam = tree_multimap(l_update, lam, cons, epsilon)
        cnt += 1
...
def admm_adam(step_size_f, step_size_g, f_fn, g_fn, rho = 1.0,
    ...
    def update(i, state, batch, weights=(1.0, 1.0)):
        ...
        def inner_step_f(k, inputs):
            ...
            mx = tree_multimap(m_update, mx, gx)
            vx = tree_multimap(v_update, vx, gx)
            mhatx = tree_multimap(mhat_upate, mx)
            vhatx = tree_multimap(vhat_upate, vx)
            gx = tree_multimap(g_update, mhatx, vhatx)
            x = tree_multimap(x_update, x, gx)
            cnt_x += 1
        ...
        def inner_step_g(k, inputs):
            ...
            my = tree_multimap(m_update, my, gy)
            vy = tree_multimap(v_update, vy, gy)
            mhaty = tree_multimap(mhat_upate, my)
            vhaty = tree_multimap(vhat_upate, vy)
            gy = tree_multimap(g_update, mhaty, vhaty)
            y = tree_multimap(y_update, y, gy)
            cnt_y += 1
            return y, my, vy, cnt_y
        y, my, vy, cnt_y = lax.fori_loop(0, num_inner_iter_g, inner_step_g, (y, my, vy, cnt_y))
        # Update w
        w = tree_multimap(w_update, x, y, w)
